...
TARGET		:=	boot
BUILD		:=	build
SOURCES		:=	source source/vendor source/parse source/util source/game
DATA		:=	data
TEXTURES	:=  textures
INCLUDES	:=
...
#---------------------------------------------------------------------------------
LIBS	:=	-lfat -l:libz.a -lwiiuse -lbte -lmad -lasnd -logc -lm -lcjson
...
#---------------------------------------------------------------------------------
load: $(BUILD)
	@echo Copying $(OUTPUT).dol to $(SDPATH)
	@cp $(OUTPUT).dol $(SDPATH)
	@echo Done.
